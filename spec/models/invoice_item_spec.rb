...
RSpec.describe InvoiceItem do
  ...
  describe 'instance methods' do 
    ...
    it "can get the best bulk discount from a merchant" do 
      ...
      bulk_discount2 = BulkDiscount.create!(quantity_threshold: 10, percentage: 20, merchant_id: merchant1.id)
      bulk_discount3 = BulkDiscount.create!(quantity_threshold: 5, percentage: 5, merchant_id: merchant1.id)
      bulk_discount4 = BulkDiscount.create!(quantity_threshold: 5, percentage: 15, merchant_id: merchant1.id)
      ...
      invoice_item4 = InvoiceItem.create!(quantity: 1, unit_price: item4.unit_price, status: "shipped", item_id: item3.id, invoice_id: invoice3.id)
      ...
      expect(invoice_item2.get_max_discount).to eq(bulk_discount4)
      expect(invoice_item3.get_max_discount).to eq(bulk_discount2) 
      expect(invoice_item4.get_max_discount).to eq(nil)  
    end
  end
end